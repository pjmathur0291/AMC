...
export default function HeroSection({ onSuccessSubmit }: HeroProps) {
  ...
  // NOTE: `onSuccessSubmit` is kept to preserve the original component contract,
  // even though the success event is currently triggered downstream.
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-25 w-full animate-in fade-in duration-700">
        <div className="max-w-2xl lg:max-w-3xl space-y-6 md:space-y-8 text-left">
          ...
          <p className="text-base sm:text-lg text-gray-700 font-sans leading-relaxed max-w-2xl">
            Start your <strong className="text-[#161D4A] font-extrabold border-b-2 border-[#FED304]">BTech or MTech</strong>{' '}
            journey at AMC Engineering College, recognized among the{' '}
            <strong className="text-[#161D4A] font-extrabold font-display">Top 5 Engineering Colleges in Bangalore (TOI)</strong>
            , with industry-ready programs and strong career support.
          </p>
          ...
          <div className="space-y-4 pt-2">
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {[
                ...
                <li key={index} className="flex items-start space-x-3 group" id={`hero-bullet-${index}`}>
          ...
          <div className="bg-white border border-[#161D4A]/10 px-6 py-4.5 rounded-2xl shadow-sm space-y-2.5 max-w-lg">
            ...
            <div className="flex flex-row flex-nowrap items-center justify-between gap-2.5">
              {[
                ...
                <div
                  key={idx}
                  className="h-16 w-16 sm:h-20 sm:w-20 bg-white border border-gray-150 rounded-xl p-2.5 flex items-center justify-center shadow-2xs hover:shadow-xs transition-shadow flex-shrink-0"
                >
                  <img src={logo} alt={`Accreditation Logo ${idx + 1}`} className="max-h-full max-w-full object-contain" referrerPolicy="no-referrer" />
                </div>
              ))}
            </div>
          </div>
